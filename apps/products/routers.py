...
@router.post('/', response_model=schemas.ReturnProduct, tags=['products'])
async def get_products(*, db: Session = Depends(get_db), product: schemas.Product):
    return crud.create_product(db=db, product=product)


@router.put('/{id}/', response_model=schemas.ReturnProduct, tags=['products'])
...
@router.patch('/{id}/', response_model=schemas.ReturnProduct, tags=['products'])
...
@router.delete('/{id}/', response_model=schemas.ReturnProduct, tags=['products'])
async def get_products(*, db: Session = Depends(get_db), id: int):
...
async def get_products(*, db: Session = Depends(get_db), cart_id: int = Depends(get_current_user), product: schemas.SetProduct):
...
async def get_products(*, db: Session = Depends(get_db), cart_id: int = Depends(get_current_user)):
